...
export default function LobbyChat({
  ...
  return (
    <section className="flex flex-col gap-2 min-h-0 flex-1 shrink">
      ...
      <div className="flex-1 max-h-[160px] overflow-y-auto border-2 border-(--game-surface-border) bg-black/20 rounded px-3 py-2 flex flex-col gap-1.5">
        {messages.length === 0 ? (
          ...
          messages.map((m, i) => (
            <p
              key={i}
              className="text-sm text-(--game-text) leading-snug break-all"
            >
